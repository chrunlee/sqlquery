	...
	_createSql (type,schemeObj,valueObj){
		...
		if(type == 'select'){
			...
			if(valueObj instanceof Array){//为数组，批量插入
				...
				sql += ') '+tempStr;
				//insert 语句nowhere
				let insertsqlobj = {sql : sql,params : params};
				if(tool.config.mysql){
					console.log(insertsqlobj);
				}
				return insertsqlobj;
			}
		}
		...
		if(tool._getLengthOfObj(scheme.where) > 0){
			...
			for(let name in scheme.where){
				...
				let whereAlias = nameOpt.table || tableAlias;
				let whereValue = nameOpt.value || nameOpt;
